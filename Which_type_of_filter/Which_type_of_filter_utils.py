...
class WhichTypeOfFilter():
    def __init__(self):
        pass
    
    def Ex1(self):
        plt.close('all')
        return FilterTypeEx(ex=1, minval=-np.pi/30, maxval=np.pi/30)

    def Ex2(self):
        plt.close('all')
        return FilterTypeEx(ex=1, minval=(np.pi - np.pi/30), maxval=-(np.pi - np.pi/30))
    
    def Ex3(self):
        return MCQ(options=['low-pass', 'high-pass', 'band-pass', 'band-stop'], correct='band-pass').display
    
    def Ex4(self):
        return MCQ(options=['𝜔0 < 𝜃', '𝜔0 ≈ 𝜃', '𝜔0 = 𝜃', '𝜔0 > 𝜃'], correct='𝜔0 ≈ 𝜃').display
    
    def Ex5(self):
        plt.close('all')
        return FilterTypeEx(ex=2, minval=(np.pi - np.pi/30), maxval=-(np.pi - np.pi/30))
    
    def Ex6(self):
        plt.close('all')
        return FilterTypeEx(ex=2, minval=-np.pi/30, maxval=np.pi/30)
    
    def Ex7(self):
        return MCQ(options=['low-pass', 'high-pass', 'band-pass', 'band-stop'], correct='band-stop').display
    
    def Ex8(self):
        return MCQ(options=['𝜔c < 𝜃', '𝜔c ≈ 𝜃', '𝜔c = 𝜃', '𝜔c > 𝜃'], correct='𝜔c ≈ 𝜃').display
...
class DraggableMarker():
    ...
    def click(self, event):
        ...
        if event.inaxes == self.ax:
            ...
            if self.ex == 1 and self.active_line == 0:
                active_point, active_line = self.get_closest(
                    event.xdata, event.ydata, skip_first=True)
                if active_point is None or active_line is None:
                    self.draggable = False
            ...
            if self.ex == 2 and self.active_line == 1:
                self.draggable = False
                data_x, data_y = self.lines[self.active_line].get_data()
                event.xdata = data_x[self.active_point]
                event.ydata = data_y[self.active_point]
    ...
    def update(self, event):
        # Adjust values to radius=self.r
        self.theta = np.arctan2(event.ydata, event.xdata)
        event.xdata = self.r * np.cos(self.theta)
        event.ydata = self.r * np.sin(self.theta)
    ...
    def get_closest(self, mx, my, skip_first=False):
...
class TypeOfFilterZPP(DraggableMarker):
    def __init__(self, ex=1, r=0.75):
        ...
        self.zero_range = widgets.IntSlider(
            value=zeros, min=0, max=zeros+10, step=2 if self.real_filter else 1, description='Zeros:', disabled=True)

        self.zero_range.observe(self.on_zero_change, names='value')
        # Poles
        self.pole_range = widgets.IntSlider(
            value=poles, min=0, max=poles+10, step=2 if self.real_filter else 1, description='Poles:', disabled=True)
        ...
        self.real_button=widgets.Checkbox(value= self.real_filter, description = "Real filter", 
                                          layout=Layout(width='50%'), disabled=True)
        ...
        if self.real_filter:
            ...
            # Update conjugates two times (two poles / zeros that need to be collapsed)
            self.active_line = 0 if self.ex == 1 else 1
            self.active_point = 0
            self.update_conjugate()
            self.active_point = 1
            self.update_conjugate()
            # Reset active line and active point
            self.active_line = 1 if self.ex == 1 else 0
            self.active_point = 0
...
#                 self.axs[0].lines[0].set_visible(False)
    ...
    def Zero_RE_Caller(self, change):
        ...
        if (self.ex == 1 and self.active_line == 0) or (self.ex == 2 and self.active_line == 1):
            change['new'] = change['old']
        if self.actual_change:
            if change['new'] != change['old']:
                # Adjust values to radius=self.r
                self.lastzeroRe = np.clip(change['new'], -self.r, self.r)
                self.theta = np.arccos(self.lastzeroRe/self.r)
                self.lastzeroIm = self.r * np.sin(self.theta)
            self.ChangeZero()
    ...
    def Zero_IM_Caller(self, change):
        ...
        if self.actual_change:
            if change['new'] != change['old']:
                # Adjust values to radius=self.r
                self.lastzeroIm = np.clip(change['new'], -self.r, self.r)
                self.theta = np.arcsin(self.lastzeroIm/self.r)
                self.lastzeroRe = self.r * np.cos(self.theta)
            self.ChangeZero()
            if self.real_filter: self.update_conjugate()
    ...
    def change_freq_res(self, init=False, redraw=False):
        ...
        if init == True:
            with self.out:
                # Gain
                self.axs[1].set_title(r'Frequency response, $\theta=' + str(np.round(self.theta/np.pi, 2)) + r'\pi$')
                self.axs[1].plot(w, h)
            ...
            h_max = np.max(h)
            # Update theta plot
            self.axs[1].lines[1].set_data([self.theta, self.theta], [0, h_max])
            self.axs[1].lines[2].set_data([-self.theta, -self.theta], [0, h_max])
            self.axs[1].set_title(r'Frequency response, $\theta=' + str(np.round(self.theta/np.pi, 2)) + r'\pi$')
...
class FilterTypeEx():
    def __init__(self, ex, minval, maxval):
        self.flag = False
        if minval > maxval:
            self.flag = True
        self.min = minval
        self.max = maxval
        self.tofzpp = TypeOfFilterZPP(ex=ex)     
        self.submit_button = widgets.Button(description='Submit', button_style='', layout={'width': '100px'})
        self.submit_button.on_click(self.check_answer)
        display(self.submit_button)
    
    def check_answer(self, value):
        if self.flag:
            if self.tofzpp.theta >= self.min or self.tofzpp.theta <= self.max or -self.tofzpp.theta >= self.min or -self.tofzpp.theta <= self.max:
                self.submit_button.button_style = 'success'
            else:
                self.submit_button.button_style = 'danger'
        else:
            if (self.tofzpp.theta >= self.min and self.tofzpp.theta <= self.max) or (-self.tofzpp.theta >= self.min and -self.tofzpp.theta <= self.max):
                self.submit_button.button_style = 'success'
            else:
                self.submit_button.button_style = 'danger'